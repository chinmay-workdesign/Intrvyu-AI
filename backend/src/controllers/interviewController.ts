...
import { runGraphPipeline, generateFeedbackReport, triggerAsyncFeedbackGeneration } from '../graph/graphEngine.js';
import { generateJSON, generateContent } from '../services/geminiService.js';
...
async function generateSpeechResponse(prompt: string, systemPrompt: string, voiceName: string): Promise<{ text: string; audioBase64: string }> {
...
export async function handleMessage(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    if (updatedState.lastEvaluation) {
      await prisma.conversationMessage.update({
        ...
        data: {
          evaluation: updatedState.lastEvaluation as any
        }
      });
    }
