import { InterviewState, EvaluationMetrics } from './state/state.js';
 ...
 */
export async function evaluationNode(state: InterviewState): Promise<InterviewState> {
...
`;
  ...
  try {
    const evaluation = await generateJSON<EvaluationMetrics>(prompt, EVALUATION_SYSTEM_PROMPT);
 ...
 */
export async function difficultyAdjustmentNode(state: InterviewState): Promise<InterviewState> {
  if (!state.lastEvaluation) return state;
 ...
 */
export async function coverageTrackerNode(state: InterviewState): Promise<InterviewState> {
  const currentTopic = state.currentTopic;
  if (!currentTopic || !state.lastEvaluation) return state;
 ...
 */
export async function followUpDecisionNode(state: InterviewState): Promise<InterviewState> {
  ...
  const topicMessageCount = state.transcript.filter(
    m => m.text.toLowerCase().includes(currentTopic.toLowerCase())
  ).length;
 ...
 */
export async function nextQuestionNode(state: InterviewState): Promise<InterviewState> {
 ...
 */
export async function runGraphPipeline(state: InterviewState): Promise<InterviewState> {
 ...
 */
export async function generateFeedbackReport(state: InterviewState, jobDescriptionText?: string, resumeText?: string): Promise<any> {
...
${transcriptString}
  ...
  return await generateJSON<any>(prompt, FEEDBACK_REPORT_PROMPT);
}

/**
 * Background / Helper to build feedback report
 */
export async function triggerAsyncFeedbackGeneration(sessionId: string): Promise<any> {
  try {
    ...
    const dummyState: InterviewState = {
      sessionId,
      interviewType: session.type as any,
      experienceLevel: session.experienceLevel as any,
      currentTopic: session.currentTopic,
      difficulty: session.difficulty,
      coverage: session.coverage as any,
      transcript,
    ...
    if (userAnalytics) {
      ...
      const trend = JSON.parse(JSON.stringify(userAnalytics.scoreTrends)) as { date: string; score: number }[];
      trend.push({
      ...
      const currentComps = JSON.parse(JSON.stringify(userAnalytics.competencies)) as Record<string, number>;
