...
export async function handleLiveSession(clientWs, sessionId) {
    ...
    let transcriptAccumulator = session.messages.map(m => ({
        speaker: m.speaker,
        text: m.text,
                    ...
                    if (currentCandidateText.trim()) {
                        ...
                        if (currentSession) {
                            ...
                            const graphState = {
                                ...
                                coverage: currentSession.coverage,
                            ...
                            if (updatedState.lastEvaluation) {
                                await prisma.conversationMessage.update({
                                    where: { id: candMsg.id },
                                    data: { evaluation: updatedState.lastEvaluation }
                                });
                            }
